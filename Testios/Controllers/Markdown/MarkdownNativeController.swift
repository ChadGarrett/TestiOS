//
//  MarkdownNativeController.swift
//  Testios
//
//  Created by Chad Garrett on 2022/02/10.
...
final class MarkdownNativeController: AppViewController {
    ...
    override func setupView() {
        ...
        self.view.addSubviewAndPinToSuperviewEdges(self.lblText, with: UIEdgeInsets.init(all: Style.padding.l))

        if #available(iOS 15, *) {
            do {
                let parsedMarkdown = try AttributedString(markdown: MARKDOWN_TEXT)

                self.lblText.attributedText = NSAttributedString(parsedMarkdown)
...
internal let MARKDOWN_TEXT: String = """
## Down
